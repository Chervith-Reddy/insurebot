...
const incidentToClaimTypeMap: Record<IncidentType, ClaimType> = {
...
const classifyWithFallback = (incidentType: IncidentType): AIClassificationResult => {
  const claimType = incidentToClaimTypeMap[incidentType] || 'unknown';
  return {
    claimType,
    confidence: 0.85,
...
export const classifyClaim = async (
  ...
  if (!apiKey || apiKey === 'your_groq_api_key_here') {
    ...
    return classifyWithFallback(incidentType);
  }

  const prompt = `You are an insurance claims classifier. Analyze the following insurance claim and classify it.
